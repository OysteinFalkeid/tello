...
class PID(Node):
    def __init__(self):
        ...
        self.Kp = np.array([0.25, 0.25, 1.0, 0.5], dtype=float)
        self.Kd = np.array([0.03, 0.03, 0.1, 0.0], dtype=float)
    ...
    def subscribe_goal(self, msg:PoseStamped):
        ...
        seconds = sec + nanosec / 1000000000
        self.delta_time = seconds - self.seconds 
        self.seconds = sec + nanosec / 1000000000
    ...
    def tune_pid(self):
        ...
        if self.z_toggle:
            ...
            self.goal_pose.pose.position.z = 0.5
            self.goal_pose.pose.orientation.w = 1.0
        
        else:
            self.goal_pose.pose.position.x = 0.0
            self.goal_pose.pose.position.y = 0.0
            self.goal_pose.pose.position.z = 0.3
            self.goal_pose.pose.orientation.w = 1.0

        self.z_toggle = not self.z_toggle
        self.get_logger().info(f"New goal: {self.goal_pose.pose.position.x, 
